...
def invalidate_includes(source_path):
    ...
    if isfile(includes_path):
        ...
        params = []
        files = [normpath(relpath(file, source_path)) for file in glob.glob(
            f"{source_path}/**/*", recursive=True)]

    with open(includes_path, "w") as includes:
        params = ["# " + key + (": " + str(params[key]).lower() if not params[key] else "") + '\n' for key in params]
        files = [file + '\n' for file in files]
    ...
    for item in sources:
        ...
        for source_path in make_config.get_paths(_source):
            ...
            if (isfile(source_path)):
                ...
                result = build_source(source_path, destination_path)

            if result != 0:
                overall_result = 1

            tsconfig_path = join(source_path, "tsconfig.json")
            declarations_name = f"{splitext(basename(source_path))[0]}.d.ts"
            tsconfig_list[declarations_name] = tsconfig_path
