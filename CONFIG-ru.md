...
│  ├─ compiledLibraries: false // включенные JavaScript библиотеки трактуются как уже скомпилированные и не собираются для получения деклараций, используйте только в случае если вам не нужна минификация библиотек или требуется их постоянное обновление
...
│
├─ sources: [] // директории для компиляции скриптов, например основного скрипта для запуска или main.js, либо сами скрипты
│  └─ {}
...
│     ├─ api: "<../../api>" // "CoreEngine", "AdaptedScript", "Preloader", "PrefsWinAPI", "Instant" (только для Instant Referrer)
│     ├─ optimizationLevel: optimizationLevel // значение между -1..9, служит для выгрузки скриптов из памяти
│     └─ shared: false // декларации будут разделены между всеми файлами проекта, актуально лишь для специфических случаев
│
