...
- [node.js](https://nodejs.org/en/) 10.15.1 или выше (для TypeScript моддинга), также должен быть установлен `tsc` версии 3.0 или выше (для этого запустите `npm install -g typescript@6`); версия ДОЛЖНА быть ниже 7.0, поскольку она [удаляет ES5 как целевую платформу](https://github.com/microsoft/TypeScript/issues/62196)
...
Есть несколько вариантов использования тулчейна через консоль или терминал. Первый, и самый простой — открыть папку *toolchain/toolchain/python* в консоли, вручную запуская находящиеся там скрипты. Вы можете добавить эту же папку в переменную среды `PATH`, здесь можно почитать как это сделать [на вашей платформе](https://www.java.com/ru/download/help/path.html).
